...
func newTestFetcher(run glabRunner) *Fetcher {
...
func TestFetch_Success(t *testing.T) {
	...
	run := func(ctx context.Context, dir string, args []string) ([]byte, error) {
		...
		return []byte(`{"iid":42,"title":"Fix the greeting","description":"It says goodbye instead of hello.","web_url":"https://gitlab.com/example/repo/-/issues/42"}`), nil
	}
	...
	if issue.URL != "https://gitlab.com/example/repo/-/issues/42" {
		t.Errorf("URL = %q, want the issue URL", issue.URL)
	}

	if gotDir != "/repo" {
		t.Errorf("glab dir = %q, want %q", gotDir, "/repo")
	}
	wantArgs := []string{"issue", "view", "42", "--output", "json"}
...
func TestFetch_EmptyID(t *testing.T) {
	f := newTestFetcher(func(ctx context.Context, dir string, args []string) ([]byte, error) {
		t.Fatal("glab should never be invoked for an empty id")
		return nil, nil
	})
...
func TestFetch_NonNumericID(t *testing.T) {
	f := newTestFetcher(func(ctx context.Context, dir string, args []string) ([]byte, error) {
		t.Fatal("glab should never be invoked for a non-numeric id")
		return nil, nil
	})

	_, err := f.Fetch(context.Background(), "GROUP-123")
	if err == nil {
...
func TestFetch_GlabFailurePropagates(t *testing.T) {
	f := newTestFetcher(func(ctx context.Context, dir string, args []string) ([]byte, error) {
		return nil, errors.New("glab: issue not found")
	})

	_, err := f.Fetch(context.Background(), "999")
	if err == nil {
		t.Fatal("Fetch returned nil error when glab failed")
	}
	if !strings.Contains(err.Error(), "issue not found") {
		t.Errorf("error = %q, want it to include glab's own error", err)
	}
}
...
func TestFetch_UnparseableOutput(t *testing.T) {
	...
	if _, err := f.Fetch(context.Background(), "1"); err == nil {
		t.Fatal("Fetch returned nil error for unparseable glab output")
	}
}
