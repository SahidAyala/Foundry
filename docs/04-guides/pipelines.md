...
- **`issue`** (`.foundry/pipelines/issue.json`) — `plan → approve-plan → implement → verify → approve-outcome → apply (target: remote-pr) → record`, repair bounded at 2 attempts, targeting `implement`. Backs `/issue <id>`, this repository's own richer starter (see [getting-started.md](getting-started.md)'s "AI code review" and ticket-provider sections). Its `plan` Step pins `"executor": "planner"` (`.foundry/executors.json` maps that name to the Gemini CLI vendor) — the one document in this repository that actually exercises the `executor` field: the analysis/planning Step runs on a different, cheaper model than `implement`'s (the default Executor, unpinned), diversifying models within a single Act rather than running the whole lifecycle against one vendor.
