...
- A model's Executor value must match an entry's *name* in `.foundry/executors.json` (e.g. an entry literally named `"gemini"`), not merely share its `vendor`. A project that names its Gemini entry `"planner"` (as this repository's own `.foundry/executors.json` does) needs a `model` pinned Step to resolve against an entry actually named after the vendor, or against `"planner"` directly via `executor` instead — `model` does not search `.foundry/executors.json` by `vendor`. **Since ADR-0013's post-ratification "Claude Code model selection" note, a project *can* register Claude Code as a named, model-pinned `.foundry/executors.json` entry** (`{"vendor": "claude", "model": "<a Claude model alias or name>"}`) — closing what used to be an unconditional "unknown executor" failure for any `model`/`preferred` naming a Claude model ID. **The safe way to use this is a direct `executor` pin** (e.g. `{"executor": "claude-fable"}`), not `model`/`preferred`: every Claude model in `executor/claude.SupportedModels()`'s own catalog shares one Executor value (`"claude"`), so `model: "claude-fable-5"` only resolves *correctly* if a project's own executors.json entry is literally named `"claude"` *and* that one entry's configured model happens to match — a project wanting more than one Claude model selectable this way would need a distinctly-named entry per model, which the catalog's shared Executor value can't distinguish between. This is the same shared-name caveat already true for Gemini above, now also reachable for Claude.
...
- The Model Registry (`model.Registry`) also carries a runtime `HealthManager` (`AVAILABLE`/`UNAVAILABLE`/`COOLDOWN`/`UNKNOWN`, plus a `reason` and `retryAt`), queryable via `Registry.Health` — ADR-0013 (Accepted), fifth increment. This is not a Pipeline/Step concept: no field on a Step reads or reports it. It is, however, consulted automatically for a Step naming two or more `preferred` (or capability-filtered) candidates: a candidate currently reported `UNAVAILABLE`/`COOLDOWN` (whose `retryAt` hasn't passed) is deprioritized behind healthier candidates before any of them is ever attempted — see "Preferred model lists and automatic failover" below. A Step naming only `model` (no `preferred`), or only `executor`, has nothing to deprioritize *to*, so its resolution is unaffected either way.
...
- **There is still no availability *probing*** — nothing makes a network call to check whether `claude-opus-4-8` is reachable before trying it. A retryable failure is still only detected by actually attempting the call and it failing. What *has* changed (ADR-0013's post-ratification "HealthManager connected" note): before the first attempt, `preferred`'s candidates are reordered so any candidate the Model Registry's `HealthManager` currently reports `UNAVAILABLE`/`COOLDOWN` (and not yet past its own `retryAt`) sorts behind healthier candidates — a soft deprioritization based on a past `Report`, not a live probe, and never a hard exclusion: if every candidate is currently reported down, Foundry still tries the first one rather than refusing the Step outright over health data that might already be stale.
- **This only actually helps once a real Executor's errors are classified.** `executor/openai` (and therefore the `"openai"`/`"openai-compatible"` vendors — Ollama, Groq, DeepSeek, GitHub Models, ...) and `executor/gemini` (Google's own `generateContent` HTTP API) now emit a classified `model.FailureError` for their documented 401/403 (authentication), 429 (rate limit), 404 (invalid model), 5xx (temporary unavailability), and context-deadline-exceeded (timeout) cases — each mapping confirmed against that vendor's own documented error taxonomy before writing it, not guessed. `executor/claude`, `executor/geminicli`, and `executor/copilotcli` remain unclassified: unlike the two HTTP-based Executors above, these wrap a local CLI subprocess whose documented error signal (a generic non-zero exit code, or — for Claude Code specifically — a `system/api_retry` stream-json event that only fires *during* the CLI's own internal retries, not as a final failure signal) does not map cleanly to `FailureClass`'s named values without a real invocation-shape change (e.g. switching to streamed JSON parsing); left unclassified rather than guessed at from free-text stderr, the same "unknown defaults to not retryable" safety this file's classified cases already rely on.
...
- **This is a narrow, static form of capability matching** — a fixed, hand-curated catalog checked once against a fixed list, not a dynamic negotiation protocol, not a scoring/ranking policy. The capability-qualified candidates are then reordered by `HealthManager` state exactly as plain `preferred` lists are (see above) — health deprioritizes among the already-capable set, it never overrides a capability requirement. See the ADR's own Context for how this relates to [ADR-0006](../03-adrs/ADR-0006-routing-and-policy.md)'s previously-deferred capability-based routing.
