...
### Piece 6 — VCS/PR publish policy — **ADR drafted**
Drafted as [ADR-0010](../03-adrs/ADR-0010-vcs-pr-integration-and-apply-targets.md) — **Status: Proposed, not ratified** (no governance process exists yet, [OQ-006](../06-open-questions/OQ-006-governance-model.md)). It keeps `Apply`'s meaning and `engine.Applier`'s contract unchanged, adds `ApplyTargetRemotePR = "remote-pr"` resolved through the same `ApplierRegistry` Piece 4 already built, and decides the three things RFC-0004 §2.5 named but didn't itself ratify: `project.Config` gains `RequireApprovalBeforeRemotePublish`/`RemotePublishTokenEnv`; `engine.PipelineRegistry` gains `SetPublishPolicy(bool)` so `Register`/`RegisterMany` refuse — at load time, never at run time — a Pipeline declaring `remote-pr` without a preceding `approve` Step when the project requires one; and a new `vcs` package (`vcs.GitHubPRApplier`) shells out to the `gh` CLI, the same subprocess posture `executor/claude` and `git apply` already establish. Piece 6 may proceed against this proposed shape — implementing `vcs.GitHubPRApplier`, `SetPublishPolicy`, the two new `Config` fields, and wiring both composition roots — the same way Piece 3 proceeded against ADR-0005 before ratification; ratifying ADR-0010 is a separate, still-blocked step. Sequenced last deliberately — the highest-risk, least-precedented piece, exactly as RFC-0003 §8 already recommended for its own §4.1.
