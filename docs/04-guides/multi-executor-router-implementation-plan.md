...
> **Executable roadmap** for [RFC-0004](../01-rfcs/RFC-0004-multi-executor-router-and-publish-policy.md)'s six sequenced pieces. This guide is the "how, in order"; RFC-0004 is the "why". Each commit below leaves the repository compiling with tests green, mirroring [M0-IMPLEMENTATION-BACKLOG.md](../archive/obsolete/M0-IMPLEMENTATION-BACKLOG.md) (historical; M0 is complete) and [interactive-session-implementation-plan.md](interactive-session-implementation-plan.md)'s discipline.
>
...
Drafted as [ADR-0005](../03-adrs/ADR-0005-executor-contract-and-capability-model.md) — **Status: Accepted** (ratified 2026-07-16 under [ADR-0000](../03-adrs/ADR-0000-governance-and-ratification-process.md)). It keeps `engine.Executor.Execute` unchanged, adds an optional `CostEstimator` interface Piece 5 can type-assert for, and rules that Capability truthfulness stays declared-not-verified while routing is explicit-pin-only. Piece 3 proceeded against this shape and has since shipped.
...
Drafted as [ADR-0010](../03-adrs/ADR-0010-vcs-pr-integration-and-apply-targets.md) — **Status: Accepted** (ratified 2026-07-16 under [ADR-0000](../03-adrs/ADR-0000-governance-and-ratification-process.md)) — and implemented against that shape, the same way Piece 3 proceeded against ADR-0005. `Apply`'s meaning and `engine.Applier`'s contract are unchanged; `remote-pr` is Foundry's first apply target that leaves the developer's own machine.
