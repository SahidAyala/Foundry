...
A Step can instead name `model` (e.g. `"model": "claude-sonnet-5"`), resolved through Foundry's own fixed Model Registry ([ADR-0013](../03-adrs/ADR-0013-model-registry.md), Proposed) rather than an `executors.json` name directly — `model` wins if both are set on the same Step. Or a Step can name `preferred`, an ordered list of Model IDs (e.g. `"preferred": ["claude-opus-4-8", "claude-sonnet-5", "gemini-3.1-pro"]`) — only the first entry is used today (no availability check or fallback yet), and it wins over `model` if both are set. See [pipelines.md](pipelines.md)'s "Pinning a Step to a Model" and "Preferred model lists" sections for the full precedence rules and a real, named limitation (models belonging to Claude Code can't resolve this way yet).
