...
A named `"claude"`-vendor entry can also set `timeout_seconds` (e.g. `{"vendor": "claude", "timeout_seconds": 600}`) to override how long a single Execute call may run before giving up — `executor/claude.NewClaudeExecutor`'s own built-in default is 5 minutes, which a large repository or a genuinely multi-file change can exceed on its own, independent of anything actually being wrong with the attempt. Omitted (the default) leaves that 5-minute ceiling unchanged. Only the `"claude"` vendor consults this field today; setting it on any other vendor's entry has no effect yet.
