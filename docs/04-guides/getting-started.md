...
|---|---|---|
| [Go](https://go.dev/dl/) 1.24+ | Building the binary | Build-time only — not required after `foundry` is installed. |
...
```json
{
  ...
  "claude-fable": { "vendor": "claude", "model": "fable" }
}
```

Then reference the name (`"gpt"`/`"flash"`/`"flash-ci"`/`"local-llama"`/`"gh-copilot"`/`"claude-fable"` above) from a Step's `executor` field in a Pipeline document — see [pipelines.md](pipelines.md). A missing `executors.json` means only the default Executor exists; nothing above is required to use Foundry at all. `claude`, `openai`, `gemini`, `gemini-api`, `copilot`, and `openai-compatible` are the supported vendors today ([ADR-0005](../03-adrs/ADR-0005-executor-contract-and-capability-model.md)); an unrecognized vendor is a clear configuration error, not a silent fallback. The `"claude"` vendor's `model` is passed straight through to the `claude` CLI's own `--model` flag (an alias like `sonnet`/`opus`/`haiku`/`fable`, or a full model name) — unlike the zero-config default Executor, which passes none, deferring entirely to Claude Code's own configured default (`.claude/settings.json`, `ANTHROPIC_MODEL`, or its own built-in default).

Any CLI-backed vendor's entry (`claude`, `gemini`, `copilot`) can also set `timeout_seconds` (e.g. `{"vendor": "claude", "timeout_seconds": 600}`) to override how long a single Execute call may run before giving up — the built-in default is 30 minutes for these agentic CLIs (see "Why the deadlines differ per vendor" below). Omitted, that ceiling is unchanged. An HTTP-API vendor's entry ignores the field: those have no configurable deadline to apply it to.

A Step can instead name `model` (e.g. `"model": "claude-sonnet-5"`), resolved through Foundry's own Model Registry ([ADR-0013](../03-adrs/ADR-0013-model-registry.md), Accepted) rather than an `executors.json` name directly — `model` wins if both are set on the same Step. Or a Step can name `preferred`, an ordered list of Model IDs (e.g. `"preferred": ["claude-opus-4-8", "claude-sonnet-5", "gemini-3.1-pro"]`) — only the first entry is used today (no availability check or fallback yet), and it wins over `model` if both are set. See [pipelines.md](pipelines.md)'s "Pinning a Step to a Model" and "Preferred model lists" sections for the full precedence rules and a real, named caveat: every Claude (or Gemini) model in that vendor's own catalog shares one Executor value, so `model`/`preferred` only resolves to the *specific* model named if your `executors.json` entry is literally named after the vendor (`"claude"`, `"gemini"`) **and** configured with that exact model — a direct `executor` pin (as in the `"claude-fable"` example above) is the safe, unambiguous way to select a specific model.
...
**Why the deadlines differ per vendor.** A CLI-backed Executor (`claude`, `gemini`, `copilot`) is an *agent*: given a prompt it reads the repository with its own tools before answering. Measured against a ~1,300-file project with Foundry's own 96 KB prompt, a single call took 98 seconds and roughly 30 tool calls on the *fastest* model; a `plan` or `implement` Step on a larger model routinely runs for many minutes. That is the Executor working, not hanging — so those vendors default to a **30-minute** ceiling, chosen to sit well past where real work finishes rather than in the middle of it. An HTTP-API vendor (`openai`, `gemini-api`, `openai-compatible`) sends one completion request and keeps a 5-minute deadline.

A timeout that lands mid-distribution is worse than no timeout: it turns working calls into failures, and each repair round pays for the same work again before hitting the same wall. Tighten it deliberately — for an unattended CI run, say — with `timeout_seconds` on any CLI-backed vendor's entry:

```json
{
  "claude-opus":   { "vendor": "claude", "model": "opus",   "timeout_seconds": 900 },
  "claude-sonnet": { "vendor": "claude", "model": "sonnet", "timeout_seconds": 900 }
}
```
