...
- **Resume continues an interrupted attempt from its last completed Step, not from scratch:** `foundry resume <act-id>` (`record/checkpoint.go`, `engine.Engine.Resume`) picks up an Act a crash or kill left mid-Pipeline, using the same Step-running logic (`engine.runSteps`) a first attempt uses. It reads from a separate, mutable checkpoint — never the immutable Record — deleted once the Act reaches a real terminal disposition, per [OQ-008](../06-open-questions/OQ-008-in-progress-act-persistence.md)'s current recommendation. Resume does not cross a repair boundary (an interruption mid-repair resumes to its already-failing verdict, not a fresh repair round) or a change to the Pipeline's declared Steps since the interrupted attempt started.

> **Unresolved (human decision required):** the *cross-version* scope of the replay guarantee — whether an Act replays identically under a future Engine version — is not yet decided. Tracked in [../00-overview/roadmap.md](../00-overview/roadmap.md) and gated by a pending ADR (see [../03-adrs/README.md](../03-adrs/README.md)). The same pending ADR also owns where in-progress Act state may live, given the Record's write-once guarantee (OQ-008).
