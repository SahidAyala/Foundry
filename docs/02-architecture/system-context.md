...
> **Unresolved, deliberately not being invested in right now:** the claim that remote/shared and large-scale (many projects, many concurrent Acts) operation requires no redesign is **plausible but unproven** — only some seams have been shown to be remoteable, and a concurrency model is not yet defined. The maintainer confirmed directly (2026-07-26) that this stays an unproven working assumption until a real multi-user/scale need appears, rather than something to design or validate speculatively now — a confirmed posture, not a resolution of the underlying question. Tracked in [../00-overview/roadmap.md](../00-overview/roadmap.md).
