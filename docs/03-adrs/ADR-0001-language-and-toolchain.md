...
4. **The extension boundary is not Go.** Per **R2**, whatever mechanism eventually carries a third-party extension must not require a plugin author to write Go — this requirement is binding, and this clause's original text asserting a specific mechanism ("a language-neutral wire format (gRPC/protobuf), decided in ADR-0008") was a premature anticipation, never a decision this ADR was entitled to make on ADR-0008's behalf. **Correction (2026-07-21, per [ADR-0008](ADR-0008-extension-isolation-and-contract-versioning.md), Accepted):** no isolation mechanism has been chosen — [extensibility.md](../02-architecture/extensibility.md) and [OQ-005](../06-open-questions/OQ-005-extension-isolation.md) had already withdrawn the gRPC/protobuf assertion as premature before this correction; ADR-0008 formally ratifies "no mechanism yet," deferred to a real third-party extension request, while R2 itself remains fully binding.
5. **Module/import path** is deferred to the resolution of the project name (RFC Open Question 7) — see Open Questions. A provisional path unblocks PR-001 and is cheap to rewrite while there are no external importers. **Resolved 2026-07-26**: the name is confirmed ("Foundry"), and the module path is now set to `github.com/SahidAyala/Foundry` (matching the repository's own VCS location, per Future ADR Dependencies item 3's own stated rule) — the provisional bare `foundry` path was rewritten across `go.mod` and every import statement in the codebase (145 files) the same day, before any external importer exists.
...
- **ADR-0008 (extension contract):** Go's out-of-process plugin ecosystem (gRPC, `go-plugin`-class tooling) remains, if a subprocess mechanism is ever chosen, the most trodden path of any candidate. **Correction (2026-07-21, per [ADR-0008](ADR-0008-extension-isolation-and-contract-versioning.md), Accepted):** no mechanism was actually chosen — this clause's "becomes low-risk" framing anticipated a decision that was never ratified; ADR-0008 explicitly defers any mechanism choice until a real third-party extension request exists.
...
- **A future WASM-based mechanism, if ADR-0008 ever chooses one.** Go-as-WASM-guest in the component model is less mature than Rust's — a real, honestly-stated cost for that path specifically. **Correction (2026-07-21, per [ADR-0008](ADR-0008-extension-isolation-and-contract-versioning.md), Accepted):** no ordering between gRPC-subprocess and WASM was actually decided; ADR-0008 defers the mechanism choice entirely, so there is no "primary path" this clause's ordering could make harder to invert.
...
- **In-process sandboxing of untrusted code is impossible.** Go offers no in-process sandbox, so untrusted community code, if it ever runs at all, cannot do so in-process — a real constraint on whatever mechanism is eventually chosen. **Correction (2026-07-21, per [ADR-0008](ADR-0008-extension-isolation-and-contract-versioning.md), Accepted):** this was not "the direction ADR-0008 chose" — no mechanism has been chosen; this remains a hard constraint on the *option set* a future mechanism decision must respect, not a decision already made.
...
2. **Hold the extension and provider boundaries language-neutral once they exist (R2 / [ADR-0008](ADR-0008-extension-isolation-and-contract-versioning.md)).** No extension mechanism or wire protocol exists yet — [ADR-0008](ADR-0008-extension-isolation-and-contract-versioning.md) (Accepted 2026-07-21) explicitly defers that choice until a real third-party extension request exists. The intent stands as a requirement for whenever one is built: a specialized component should be writable in another language without touching the kernel — the pressure-release valve this clause names is aspirational today, not yet a built mechanism.
...
- **ADR-0008 (Extension contract):** **Correction (2026-07-21, per [ADR-0008](ADR-0008-extension-isolation-and-contract-versioning.md), Accepted):** ADR-0008 does not discharge R2 by defining a language-neutral boundary — it explicitly defers that discharge, ratifying "no mechanism yet" until a real third-party extension request exists. R2 itself remains binding throughout; only its concrete mechanism is deferred.
...
3. ~~**Module/import path.**~~ **RESOLVED 2026-07-26** — see Decision clause 5 above: `github.com/SahidAyala/Foundry`, set the same day the name was confirmed, before any external importer exists.
