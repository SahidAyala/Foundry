...
5. **Module/import path** is deferred to the resolution of the project name (RFC Open Question 7) — see Open Questions. A provisional path unblocks PR-001 and is cheap to rewrite while there are no external importers. **The name itself is now resolved** (RFC-0001 Open Question 7, 2026-07-26) — the module/import path decision this clause defers is still open; see Future ADR Dependencies item 3.
...
3. **Module/import path.** Was blocked on the project name (RFC Open Question 7); the name itself is now resolved ("Foundry", 2026-07-26). No M6 public SDK or external importer exists yet, so the canonical path can still be set cheaply — but *what* it should be (e.g. matching the repository's own VCS location) remains a distinct, still-open decision this item continues to track.
