...
| [Executor contract & capability model](ADR-0005-executor-contract-and-capability-model.md) — **drafted, Proposed, not ratified** | The normalized contract every Executor implements; capability negotiation. Drafted per [RFC-0004](../01-rfcs/RFC-0004-multi-executor-router-and-publish-policy.md) §2.3 (invocation-shape, cost/telemetry, and capability-truthfulness differences across vendors) | Model-agnostic execution |
...
| [VCS/PR integration & Apply targets](ADR-0010-vcs-pr-integration-and-apply-targets.md) — **drafted, Proposed, not ratified** | Whether `Apply` may target a remote pull request, not only a local branch; the Authority/accountability model once a commit + PR can happen without a second human review point. Drafted per [RFC-0004](../01-rfcs/RFC-0004-multi-executor-router-and-publish-policy.md) §2.5 and [RFC-0003](../01-rfcs/RFC-0003-interactive-assistant-and-multi-executor-pipelines.md) §4.1: a `remote-pr` apply target plus a project-level, load-time-enforced approval policy (`PipelineRegistry.SetPublishPolicy`) | The `Apply` trust boundary (I4/I5) the moment Foundry's first action leaves the developer's machine |
