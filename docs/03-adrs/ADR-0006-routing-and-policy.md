...
3. **Capability-based negotiation (RFC-0002 §7 layer 2) remains explicitly deferred — not built.** Its own named trigger — "once ≥2 real Executors advertise the same Capability in production" — has not fired: exactly two real vendor Executors exist (`executor/claude`, `executor/openai`), and no shipped Pipeline document declares two Executors competing for one Capability tag. This ADR does not build a cost/latency/quality/privacy weighting policy speculatively; the trigger, not a calendar date or this ADR's existence, decides when it gets built.
   ...
   **Status note, added 2026-07-24, updated 2026-07-27 — not a revision of the Decision above, which remains this ADR's accepted text as written:** [ADR-0013](ADR-0013-model-registry.md) (drafted 2026-07-24, **accepted 2026-07-27**) implements a static, narrow form of capability-aware model resolution (its Decision 11 — filtering candidate models by declared `Capabilities` before selection) alongside a Model Registry now cataloguing six real vendor models across `executor/claude`/`executor/geminicli`/`executor/openai`/`executor/copilotcli`, at the maintainer's own direct and explicit request. This is real code, real tests, genuinely running today, and — as of ADR-0013's own ratification — a formally accepted decision, not merely proposed code awaiting a verdict. That ratification is itself the maintainer's explicit revisiting this note asked for: the maintainer confirmed directly that ADR-0013's Decision 11 is understood as, and intended as, deliberately exercising this Decision's own deferred capability-based routing ahead of its own named trigger firing in the general case — a choice the maintainer made, not an oversight this ADR should have caught. This Decision's own factual premise ("the trigger has not fired," in the general, any-Pipeline sense) still stands as written; ADR-0013 is the maintainer's explicit, deliberate exception to it, not a silent contradiction.

4. **Failover (RFC-0002 §7 layer 3) remains deferred alongside negotiation, for the same reason, and is built together with it rather than separately.** An availability fallback chain has no real shape to take when there is no second candidate `Executor` per Capability to fail over *to* — RFC-0002 §7 already sequences failover after negotiation, and this ADR does not reorder that. **Same status note as Decision 3 above applies**: [ADR-0013](ADR-0013-model-registry.md)'s sixth increment implements automatic failover across a Step's `preferred[]` list — real code, now formally accepted (2026-07-27), and not a revision of this Decision's own accepted text.
