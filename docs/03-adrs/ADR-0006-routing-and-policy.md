...
3. **Capability-based negotiation (RFC-0002 §7 layer 2) remains explicitly deferred — not built.** Its own named trigger — "once ≥2 real Executors advertise the same Capability in production" — has not fired: exactly two real vendor Executors exist (`executor/claude`, `executor/openai`), and no shipped Pipeline document declares two Executors competing for one Capability tag. This ADR does not build a cost/latency/quality/privacy weighting policy speculatively; the trigger, not a calendar date or this ADR's existence, decides when it gets built.
   ...
   **Status note, added 2026-07-24 — not a revision of the Decision above, which remains this ADR's accepted position until the maintainer explicitly revisits it:** [ADR-0013](ADR-0013-model-registry.md) (Proposed, unratified, drafted the same day) has, at the maintainer's own direct and explicit request, begun implementing a static, narrow form of capability-aware model resolution (its Decision 11 — filtering candidate models by declared `Capabilities` before selection) alongside a Model Registry now cataloguing six real vendor models across `executor/claude`/`executor/geminicli`/`executor/openai`/`executor/copilotcli`. This is real code, real tests, genuinely running today — but it does not, by itself, retroactively make this Decision's factual premise ("the trigger has not fired") false as *this* ADR's own accepted text: that premise stands as written until the maintainer ratifies an update to it. ADR-0013's own Context and Review Checklist name this tension directly and ask the maintainer to confirm it explicitly before ratification, rather than leaving a future reader to discover the mismatch unexplained.

4. **Failover (RFC-0002 §7 layer 3) remains deferred alongside negotiation, for the same reason, and is built together with it rather than separately.** An availability fallback chain has no real shape to take when there is no second candidate `Executor` per Capability to fail over *to* — RFC-0002 §7 already sequences failover after negotiation, and this ADR does not reorder that. **Same status note as Decision 3 above applies**: [ADR-0013](ADR-0013-model-registry.md)'s sixth increment implements automatic failover across a Step's `preferred[]` list — real code, not yet ratified, and not a revision of this Decision's own accepted text.
