...
- **Accepted (6):** [ADR-0000](../03-adrs/ADR-0000-governance-and-ratification-process.md) (governance), [ADR-0001](../03-adrs/ADR-0001-language-and-toolchain.md) (Go), [ADR-0004](../03-adrs/ADR-0004-reusable-act-template-format-and-evolution-policy.md) (Reusable-Act template format — ratified and implemented 2026-07-20), [ADR-0005](../03-adrs/ADR-0005-executor-contract-and-capability-model.md) (Executor contract — implemented), [ADR-0010](../03-adrs/ADR-0010-vcs-pr-integration-and-apply-targets.md) (VCS/PR — implemented), [ADR-0009](../03-adrs/ADR-0009-cli-and-output-contract.md) (CLI & output contract — ratified and implemented 2026-07-20). Ratifies the interactive-primary product shape, a pre-1.0 "change freely, but never silently" policy for the flag CLI and slash commands, the existing exit-code convention, and declines a JSON output mode for now; fixes the phantom `/help` banner reference and renames `PipelineProvider`→`PipelineSource` (RFC-0003 §5's flagged terminology tension) — see the RFC-0003 row above and the ADR's own Review Checklist.
- **Drafted, pending ratification (0):** none currently.
- **Backlog, implementation-relevant:** "Routing & policy (ADR-0006)" is the remaining backlog row §2 above names as an actual pending-implementation gap (capability-based routing beyond explicit-pin is unbuilt) — it now inherits a settled document format from ADR-0004 rather than an implicit one.
