...
| [RFC-0002](../01-rfcs/RFC-0002-pipeline-execution-runtime.md) — Pipeline execution runtime | No | **~85%.** Phases 1–6 and 8 shipped: Step trace on `Act`, `Strategy`/`PipelineStrategy`, Approve/Record as declared Step kinds, `FixedStrategy` fully retired, Router (explicit-pin), interactive shell. Both of Phase 0's prerequisite ADRs now drafted: [ADR-0005](../03-adrs/ADR-0005-executor-contract-and-capability-model.md) (accepted) and [ADR-0004](../03-adrs/ADR-0004-reusable-act-template-format-and-evolution-policy.md) (drafted 2026-07-20, Proposed — §3 below). | **Phase 7** — capability-based negotiation/failover (cost/latency/quality weighting) — deliberately deferred, no real multi-Executor-per-Capability case yet. [ADR-0004](../03-adrs/ADR-0004-reusable-act-template-format-and-evolution-policy.md) still needs ratification, and its Decision 4 (strict unknown-field rejection) is not yet implemented in code. |
...
- **Drafted, pending ratification (1):** [ADR-0004](../03-adrs/ADR-0004-reusable-act-template-format-and-evolution-policy.md) (Reusable-Act template format & evolution policy) — drafted 2026-07-20. Ratifies the Pipeline document as the sole reusable-Act artifact, JSON-only wire format, no version field yet, and a switch to strict unknown-field rejection (not yet implemented in code — see §2 above). Status: Proposed.
...
- **2026-07-20** — Drafted [ADR-0004](../03-adrs/ADR-0004-reusable-act-template-format-and-evolution-policy.md) (Reusable-Act template format & evolution policy), satisfying RFC-0002 Phase 0's second prerequisite ADR. Status: Proposed, not yet ratified.
